...
class AppBloc extends Bloc {
  ...
  Stream<List<Participant>> get outParticipantsList =>
  ...
  createRaffle(String name, String minValue, String maxValue,
      List<Combination> combinations) {
    _raffleController.sink.add(Raffle(
        name: name,
  ...
  bool addParticipant(Participant participant) {
    ...
    list.forEach((element) {
      if (element.name == participant.name &&
          element.email == participant.email) {
        exists = true;
  ...
  List raffleFriends() {
    ...
    if (list != null) {
      ...
      for (Participant item in list) {
        ...
        combinations
            .add(Combination(origin: item, friend: friends[randomFriend]));
  ...
  bool verifyExistInList(List<Combination> list, Participant a, Participant b) {
    list.forEach((element) {
      if (element.friend.verifyEquals(a) && element.origin.verifyEquals(b)) {
        return true;
      } else if (element.friend.verifyEquals(b) &&
          element.origin.verifyEquals(a)) {
        return true;
      }
    });
    return false;
  ...
  loadRaffles() async {
    ...
    print(historic);
    if (historic != null) {
      List<Raffle> historicList =
          historic.map((e) => Raffle.fromMap(jsonDecode(e))).toList();
      _historicListController.sink.add(historicList);
    }
    if (list != null) {
      List<Raffle> listRsp =
          list.map((e) => Raffle.fromMap(jsonDecode(e))).toList();
  ...
  mail.Message createEmail(Combination combination, Raffle data) {
    var body = "Ola ${combination.origin.name}\n"
        ...
        "Com valor mínimo \$ ${data.minimumValue} e valor máximo \$${data.maxValue} \n"
        "O seu amigo secreto é ${combination.friend.name}, A seguir os dados de seu amigo\n"
        "email: ${combination.friend.email}\n "
        "Endereço: ${combination.friend.address.street} n° ${combination.friend.address.number}\n"
    ...
    final message = mail.Message()
      ..from =
          mail.Address("virtualamigooculto@gmail.com", 'Amigo oculto virtual')
